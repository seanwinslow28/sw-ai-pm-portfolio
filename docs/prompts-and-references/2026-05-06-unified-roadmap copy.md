...
- **Default:** Ship one MCP server (`intent-engineering`) by 2026-05-25, then attach short (~400-word) `EXPLANATION.md` files to existing Code Brain artifacts in parallel. This absorbs all three positions because the MCP server *is* a publication of an existing artifact, with the explanation attached to it.
...
|---|----------|------------------|---------------------------------------------|
| 1 | Code Brain v3.20.0 (117 skills, 13 hooks, 14 SDK agents, 7 active) | **Public** — needs sanitization first (3 Block-named skills) | A README "Why this exists" section in Karpathy/Nate framing + one 5-min Loom narrating one live agent run |
...
6. `python3 scripts/validate.py` in the code-brain repo returns ≤60 warnings / 0 errors (v3.30.1 baseline preserved).
...
Two new Astro routes in `sw-ai-pm-portfolio`: `/architecture/index.astro` (landing page listing scorecard + future architecture artifacts) and `/architecture/vault-scorecard.astro` (renders `vault/SCORECARD.md`). Build-time read pattern: at site build, `scripts/build_fetch_scorecard.mjs` curls the latest `SCORECARD.md` from the code-brain repo's raw GitHub URL and writes it into `src/content/architecture/vault-scorecard.md`; Astro renders it with the V3 design tokens. Wire `/architecture/` into the main nav. Verify the ER diagram embeds correctly (use `astro-mermaid` integration if needed, ~5 min add). ~3 hours.
...
- **Gives the Code Brain Loom (if Sean records one) a 5-segment structure** — one per Nate test, with the SCORECARD as the visual spine.
...
**- [ ] Step 5: Honest gaps.** Where the Code Brain is NOT yet compliant: no formal post-market monitoring report cadence, no Annex IV-conformant template, no Article 13 user instructions. Name each. This honesty is the artifact's credibility.
...
**- [ ] Stop adding new skills to the Code Brain until June 11.**
...
**Compounding payoff:** Makes vault-knowledge-mcp (Task 10) launch self-justifying. Gives the Code Brain Loom a 5-segment structure. Foundation for the FDE interview answer "how do you think about agent memory."
