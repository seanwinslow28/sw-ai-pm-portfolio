...
description: Codex skill creation and optimization assistant. Guides building effective SKILL.md files with proper YAML frontmatter, progressive disclosure, trigger phrase optimization, and reference file architecture. Use when creating a new skill, improving an existing skill description, debugging why a skill does not auto-load, or structuring a skill directory.
---
...
Create, debug, and optimize Codex skills that auto-load reliably and deliver domain expertise without wasting context tokens. Follow the progressive disclosure architecture: metadata triggers loading, SKILL.md provides workflow, and references/ holds deep content Codex reads only when needed.
...
Codex: [Uses skill-system-mastery] Creates the directory structure:
...
Codex: [Uses skill-system-mastery] Checks:
...
Codex: [Uses skill-system-mastery] Applies progressive disclosure:
...
skill-name/
  ...
  scripts/              # OPTIONAL - executable code Codex runs via Bash
  references/           # OPTIONAL - docs Codex reads on demand via Read
...
description: What this does AND when Codex should load it. Embed trigger phrases naturally.
---
```
...
| `description` | Yes | The ONLY field Codex reads to decide whether to load. Must include trigger phrases. |

### Writing Effective Descriptions

The description controls auto-loading. Codex reads it before every user turn and decides whether to load the full SKILL.md.
...
When a description summarizes the skill's workflow, Codex may follow the description instead of reading the full skill body. This creates a shortcut that bypasses important details.

```yaml
# BAD: Summarizes workflow - Codex takes shortcut
...
| 3. Resources | references/ and scripts/ | Variable | Loaded only when Codex decides it needs them |
...
In SKILL.md, always tell Codex when to read each reference:
```markdown
...
5. `## Examples` - 2-3 user/Codex dialog exchanges
...
Apply RED-GREEN-REFACTOR to skill creation to ensure the skill actually changes Codex's behavior:

### 1. RED: Baseline Without Skill
Test what Codex does without the skill loaded. Use a subagent so your main context stays clean:

```
Launch a subagent WITHOUT the skill loaded.
Give it the exact prompt a user would type.
Record Codex's default behavior — this is your baseline.
```
...
Test with pressure scenarios — prompts where Codex might rationalize skipping the skill's guidance:
...
If Codex bypasses the skill under pressure, add explicit loophole closers (see Rationalization Prevention below).

## Rationalization Prevention

For discipline-enforcing skills (verification, checklists, review gates), Codex will rationalize skipping steps under perceived time pressure. Counter this with explicit patterns:
...
If Codex's output contains these phrases, the skill's discipline gates are being bypassed:
...
6. **Run debug mode**: `Codex --debug` shows skill triggering decisions in logs

## Component Selection Guide

| Need | Use | Why |
|------|-----|-----|
| Teach Codex HOW to do something | **Skill** | Injects procedural knowledge into context |
