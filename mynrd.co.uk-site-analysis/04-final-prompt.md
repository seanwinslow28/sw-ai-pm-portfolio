# Final Prompt — Sean Winslow Portfolio 2.0

AUDIT_MODE: high-fidelity
Quality checklist: PASSED (2026-05-08)
...
- **Core Checklist:** all items ✓
  - Named aesthetic identity ✓ ("Pencil-Test Editorial × Vercel-Grade Engineering")
  - Complete design system ✓ (semantic palette w/ light+dark hexes, 6 typography roles, texture spec)
  - Every section has a poetic component name ✓ (Sticky Origin Frame, Manifesto Tear-Sheet, Pencil-Test Filmstrip, Magician's Manifesto, Earn-Your-Applause Beat, Paper-Plane Mailto Send, Inverting Pen, Reel-Change Toggle, Animator's Pencil-Test Edges)
  - Every interactive component has implementation-level behavior spec ✓
  - At least one cubic-bezier or timing value per animated component ✓ (expo.out, quart.out, cubic-bezier(0.4, 0, 0.2, 1), 700ms, 350ms, etc.)
  - Color hierarchy preserved structurally ✓ (Outro teal → user Primary; Cream paper → user Background; Ink role mapped to user's teal-ink for warm-paper feel)
  - Technical requirements section explicit ✓ (Astro 5 / React 18 / Tailwind v4 / GSAP 3.12 / Lenis 1.1 / lottie-web 5.12, plus lifecycle, font loading, image sources, file structure)
  - Execution directive as final line ✓

- **HF Fidelity Domains:**
  - Layout ✓ — ASCII wireframes for all 6 sections + cross-cutting components
  - Visual Composition ✓ — Hero composition map (5 elements: H1, badge, lottie, bg, ambient tear) preserved; Outro composition map (18 elements: H3 + 16 marquee tiles + bg + ambient edges) preserved
  - Animation ✓ — t=Xms timelines for hero title stagger, hero circle rotation, hero lottie idle, intro per-line reveal, brand cycler, gallery tile hover, outro marquee parallax, outro title sparkle/pulse, footer mailto plane-flight, custom cursor lerp, dark mode reel-change
  - Interaction ✓ — DEFAULT→HOVER property diff tables for Gallery Tile, Footer Mailto Link, Hero Circle, Custom Cursor, Social Icons
  - Design System ✓ — full palette + 6 typography roles + drama ratio (8.5× display-to-meta, 1.9× hero-to-body, -0.15px tracking) preserved
  - Technical ✓ — stack, lifecycle, scroll setup, font loading, image sources all specified

- **Zero Generic Language scan:** passed (no "some animation", "nice hover", "smooth transition", "add padding", "use a gradient", "make it feel premium", "add content here")

---

## The Verified Replication Prompt

Paste-ready. The block below is identical to `03-replication-prompt.md` and follows the canonical 5-part hierarchy: Role + Aesthetic Identity → Core Design System → Component Architecture → Technical Requirements → Execution Directive.
...
| **Ink**              | *Press Ink*          | `#0A3E42`      | `#FFF9F0`     | Primary type — light mode uses Primary teal AS the type color (NOT pure black) for the warm-ink-on-paper feel; dark mode flips to cream |
...
**Anti-pattern:** Do NOT use pure black `#000000` anywhere. Use `#0A3E42` for ink in light mode — warm-teal ink on warm cream paper IS the visual identity.

### Typography

Default to free Google equivalents (self-hostable); provide a single labeled font-face swap point if Sean licenses Pangram Pangram originals (PP Editorial New + PP Neue Montreal) later.

| Role            | Font Family (default)      | Licensed Swap-In         | Weight | Size       | Line-Height | Tracking   | Notes                                                                          |
| --------------- | -------------------------- | ------------------------ | ------ | ---------- | ----------- | ---------- | ------------------------------------------------------------------------------ |
| Hero Display    | **Newsreader** (Google)    | PP Editorial New         | 400    | `85px` (clamp 56→85→104) | `93.5px` (1.1) | `-0.15px` | The hero H1. Tracking is critical at 85px.                              |
| Outro/Footer    | **Newsreader**             | PP Editorial New         | 400    | `120px` (clamp 72→120→160) | `132px` (1.1) | normal     | Closing line + footer "Let's work together"                              |
| Body Quote      | **Newsreader**             | PP Editorial New         | 300    | `45px` (clamp 28→45→56) | `1.29` (~58px) | normal     | Intro and About. Weight 300 — NOT 400. Italic for emphasis.              |
| Italic Inline   | **Newsreader Italic**      | PP Editorial New Italic  | 300    | inherits   | inherits    | normal     | Inside `<em>` — discipline list, named concepts.                         |
...
| Mono / Frame No | **Geist Mono** (Vercel)    | PP Neue Mono             | 400    | `12px`     | normal      | `0.02em`   | Frame counter labels (`1A`, `2A`…), the Vercel-engineering signal.       |

**⚑ Drama Ratio (PRESERVE — single highest-leverage typographic axis):**
- **120px serif at weight 400 ↔ 14px sans at weight 400 = 8.5× scale jump.** Do NOT soften.
- **85px hero serif weight 400 ↔ 45px body serif weight 300 = 1.9× ratio + weight contrast.** NEVER set body in weight 400.
- **`-0.15px` tracking on the hero** — subtle but load-bearing.

### Texture System

- **Noise/grain:** None on surfaces. Texture comes from (a) **torn-paper PNG transitions** between sections in light mode, (b) **film-reel sprocket-hole edges** between sections in dark mode (animation reel perforations), and (c) **hand-drawn ink-line illustrations** (no fill).
- **Border-radius:** `0` everywhere. NO `rounded-*` Tailwind utilities.
- **Shadow system:** None. Hard rule: no `shadow-*` Tailwind utilities anywhere. Adding shadow drift breaks the brand.
- **Frame-number motif (cross-cutting):** Every section gets a small Geist Mono `12px` label in its upper-left corner: `01A · HERO`, `02A · WORK`, `03A · ABOUT`, `04A · CONTACT`. Reinforces the animator's pencil-test narrative.
...
║ Height: auto (3-col grid × N rows; target 9–18 tiles)                       ║
║ BG: Surface                                                                 ║
║ Layout: CSS Grid 3 cols · gap 52px · tiles 1fr × aspect-ratio 11/14.5       ║
╠═════════════════════════════════════════════════════════════════════════════╣
...
║ Behind H3: parallax marquee of project frame stills (filmstrip callback)    ║
╠═════════════════════════════════════════════════════════════════════════════╣
...
OVERLAPPING sections (preserve exactly):
- **HERO ↔ INTRO**: Intro starts at scroll 100vh in flow, but its top "tear" is `position: absolute; transform: translateY(-107px)` so it bleeds upward into the hero. Hero title scrolls UNDER the tear.
...
.hero__circle-wrapper    → <a href="mailto:hi@seanwinslow.com">
                           ...
                           icon centered. Black ink stroke in light mode; cream in dark.
                           Continuous rotation: gsap.to(el, {rotation:360, duration:12,
                           ease:'none', repeat:-1}).

.hero__lottie-anim       → Hand-drawn SW character at hybrid desk (Lottie JSON ~30KB).
                           Same character reappears in footer (visual continuity).
```
...
TRANSITION A→B:
  ...
  Visual masking: intro top-edge (translateY -107px) covers the seam
...
Library: GSAP 3 + SplitText (or custom split helper)
TIMELINE:
  t=0ms      .anim-char[0]      FROM: opacity:0, yPercent:100  TO: opacity:1, yPercent:0  DUR:600ms EASE:expo.out
...
│           │  2500ms with a 350ms crossfade through      │                │
...
TIMELINE:
  t=0ms       .anim-line[0]   FROM: yPercent:60, opacity:0  TO: yPercent:0, opacity:1  DUR:700ms EASE:quart.out
  t=80ms      .anim-line[1]   TO: same  DUR:700ms
...
TRANSITION A→B:
  ...
  Element 1: outgoing  opacity 1→0  350ms ease
  Element 2: incoming  opacity 0→1  350ms ease (delay 175ms — partial overlap)
...
Tile structure: .gallery-item > .gallery-item__wrapper(overflow:hidden) >
                .gallery-item__link(<a> href to /work/[slug]) >
                <picture>|<video> object-fit:cover

NO ON-TILE TITLES. Project identity is conveyed through media.
Hover reveals project name via custom cursor preview.

Tile count: target 9–18 real projects. If fewer than 6 exist, pad with
pencil-test "in progress" frames (registration-mark border + frame number
+ "1B / IN PROGRESS" mono label) to maintain grid rhythm.
```

**Property Diff Table — Tile Hover:**
...
DURATION: 700ms (slow — documentary zoom, not UI bounce)
⚑ SPECIAL BEHAVIOR: NO box-shadow on hover. NO border. NO color overlay.
The cursor IS the affordance. The 700ms duration is load-bearing.
```
...
│                                                                          │
│                       .about__svg--regmark                               │
│                       (registration mark "+", abs top, center, ~80×80)   │
│                                                                          │
│         ┌──────────────────────────────────────────────────┐             │
...
│         └──────────────────────────────────────────────────┘             │
│                                                                          │
│                                       .about__svg--bulb (~95×91 abs r)   │
│                                                                          │
│  .about__svg--pencil (~82×84 abs left bottom — animator's pencil w/ ticks)│
│                                                                          │
│ ╱╲╱╲╱╲ Section bottom edge: white tear (light) / sprocket (dark) ╱╲╱╲    │
└──────────────────────────────────────────────────────────────────────────┘

Decorative SVGs: black-on-cream hand-drawn ink, no fill, ~80–100px each.
Animation: stagger fade-up + 400ms entrance on scroll-enter, then idle
wobble (rotate ±3° sine over 4s) for the alive hand-drawn feel.
```
...
**Layout (Composition Map preserved — 18 elements: H3 + 16 marquee tiles + bg + ambient edges):**
...
│                         Items: pencil-test frame stills from project     │
│                         archive — same media as Work tiles, repeated.    │
│                                                                          │
│              ┌──────────────────────────────────────────┐                │
...
│              │ wrapper, ~97px) — starburst around word.  │                │
│              │                                           │                │
...
GSAP: gsap.to('.infinite-grid', {y:-1400, ease:'none',
       scrollTrigger:{trigger:'.outro', start:'top bottom', end:'bottom top',
                      scrub:true}})
LOOP: no (single-pass parallax — does NOT actually loop, despite class name)
```
...
GSAP: single gsap.timeline() chains lines → svg draw-on → pulse loop
```
...
│  │ Social icons: 13×13 ink SVGs, no fill                            │    │
│  │   hover: scale(1.15) opacity 0.85→1, 200ms ease-out              │    │
│  └──────────────────────────────────────────────────────────────────┘    │
└──────────────────────────────────────────────────────────────────────────┘
```

**Property Diff Table — Footer Mailto Link:**
...
⚑ SPECIAL BEHAVIOR: paper plane "takes off" toward upper-right while ↗ arrow
nudges in the same direction — coordinated flight. The H3 itself does NOT move;
only its letter-spacing tightens slightly so the line "leans in."
```
...
STATES:
  ...
  STATE-LINK       (hover link):  span scales to 60–80px circle, mix-blend-mode:difference
  STATE-TILE       (hover tile):  span stays 6×6 dot; cursor-images[N] showing project preview (250×N)
...
⚑ SPECIAL BEHAVIOR: mix-blend-mode:difference on link hover INVERTS the color
of whatever it overlays (cream text becomes teal when the cursor passes). This
is the single most defining ambient interaction on the page — preserve without
exception.
```
...
Form: small chip (~96×40) showing a frame counter in Geist Mono
States:
...
Transition LIGHT→DARK (user clicks chip):
  ...
  t=200ms   Section edges (torn-paper PNGs) crossfade out;
...
ACCESSIBILITY: prefers-reduced-motion → mode change is INSTANT (no crossfade),
                but the frame-counter still ticks (one digit-roll, ~200ms).
...
⚑ Edges are non-negotiable. A flat section-color step instead of a tear/sprocket
= brand failure.
```
...
TECHNICAL REQUIREMENTS
  ...
  Animation:            GSAP 3.12+ (registerPlugin ScrollTrigger; SplitText if licensed,
                        ...
                        lottie-web 5.12+ for hero/footer character (lazy via client:visible)
                        prefers-reduced-motion guard at GSAP boot:
                          if (matchMedia('(prefers-reduced-motion: reduce)').matches) {
                            gsap.globalTimeline.timeScale(1000); ScrollTrigger.disable();
                          }

  Scroll:               Lenis 1.1+ smooth scroll. Mount in BaseLayout root.
                        Bridge to GSAP:
  ...
  Animation Lifecycle:  Inside React islands: useGSAP(() => { /* setup */ }, { scope: ref })
                        from @gsap/react. Auto-cleanup; no manual revert.
                        For Astro static pages:
  ...
  Hover Implementation: CSS transitions are the default for hover (transform,
                        ...
                        chains multiple elements (e.g., footer plane+arrow).
                        DO NOT use CSS :hover for custom-cursor reactions —
                        those are JS-driven via data-cursor-* attrs.

  Custom Cursor:        React island (client:idle):
                        ...
                        Hide on touch: @media (pointer: coarse) { .cursor { display:none; } }
                        Restore native cursor: html { cursor: auto; } (do NOT cursor:none)

  Font Loading:         Self-hosted via Fontsource (preferred) or Google Fonts initial:
                          Newsreader: 300, 300italic, 400, 400italic
                          Geist Sans: 400, 500
                          Geist Mono: 400
                        font-display: swap. Preload Newsreader 400 woff2 in <head>.

  Image Sources:        Development placeholders (BEFORE Sean's real assets):
                          - Hero character: hand-drawn SVG (commission/generate; reference
                            mynrd hero/footer character style)
                          - Case-study tiles: Unsplash queries —
                            "minimal product UI screenshot", "abstract editorial design",
                            "design tool interface". High-contrast, isolated subject.
                          - Decorative SVGs (registration mark, light bulb, pencil, paper plane):
                            hand-drawn ink line, no fill, ~80–100px, stroke-width 2 at 100px viewport
                            (use vector-effect:non-scaling-stroke).
                        Replace ALL placeholders with real project frame stills before launch.
                        Real assets: /public/work/[slug]/{cover.jpg, cover.mp4, cursor-preview.jpg}

  Accessibility:        - Color contrast clears WCAG AA in BOTH modes
                          (Ink #0A3E42 on #FFF9F0 ≈ 7:1; #FFF9F0 on #0A0E0F passes)
                        - All decorative SVGs aria-hidden="true"
                        - Character illustrations have meaningful alt text or aria-label
                        - Custom cursor sits ON TOP of native cursor — does NOT replace it
                        - Frame-counter labels are real text (not images)
                        - prefers-reduced-motion handled at GSAP boot
                        - Mailto link works without JS

  Performance:          - Astro builds static HTML; first paint < 1s on 3G fast
                        - Hydration: client:idle for cursor/toggle/cycler;
                          client:visible for hero Lottie
                        - Case-study videos: <video preload="none" poster="cover.jpg" muted loop
                          playsinline>; play via IntersectionObserver at 50% viewport
                        - Images: Astro <Image /> with format="webp" + width descriptors
                        - Font woff2 only
                        - Lighthouse target: 95+ desktop, 90+ mobile
...
- `plans/01-site-dna.md` — Forensic audit of mynrd.co.uk (high-fidelity)
- `plans/02-brand-interview.md` — Sean's brand answers + reconciled section plan
- `plans/03-replication-prompt.md` — Synthesized replication prompt
- `plans/04-final-prompt.md` — Quality-checked, paste-ready final prompt (this file)

If the first build attempt is incomplete or off-target, run the iterator (5-pass corrective cycle) and the output will be saved to `plans/05-iterator.md`.
