...
|---|---|---|---|
| **Home hero statement** | *"AI Product Manager"* | LOCKED 2026-08-08 (sidecar L6) | Statement hero: the tagline and the "/ AI PRODUCT MANAGER" tag are both dropped; under the name sits the plain statement. "AI Product Manager" is what the hiring market actually queries (the meta-description lock below already uses it as SEO scaffolding); Full Stack framing is argued by the work, not claimed in a title. The dateline strip + character animation survive untouched as the screen-one differentiators — any "simplification" that trims them is the template trap. Flow: who-I-am → work → how-I-think. Lives in `src/lib/site.ts` `HERO_STATEMENT`. |
| **About hook (home teaser editorial line + About PullQuote)** | *"I make agents think the way I do."* | LOCKED 2026-08-08 (sidecar L6) | The catchy line moves off the hero to the About teaser at the bottom of home, with the read-more CTA as its affordance — the claim on home, one click, and NowPulse's live data greets you on About. The byte-identical lockstep contract MIGRATED from hero↔PullQuote to teaser↔PullQuote, now enforced by construction via the shared `ABOUT_HOOK` constant in `src/lib/site.ts` (the old comment-only contract drifted once, audit 2026-06-10). Supersedes the teaser's prior locked editorial line *"A man, a pencil, an agent fleet. Same person, different tools."* (home-about-teaser-spec §2). |
| **Home hero tagline** | *"Product Manager. The agents handle the loops. I handle the taste."* | SUPERSEDED 2026-08-08 by the statement hero (sidecar L6) — was LOCKED 2026-05-18 (revised); word swap 2026-05-20 (`math` → `loops`) | Vonnegut three-sentence flat. States the PM role explicitly (no "figure it out down the line"). "Taste" is the industry-correct word for the irreducible-human-contribution in the AI era (Karpathy / Linear / Vercel / Anthropic register). The loops/taste contrast names the human-agent relationship as it actually is — agents run the iteration loops at scale (Karpathy-coded "agentic loops"; code loops + feedback loops + inference loops), the human keeps the judgment. Passes the parallel-lineage rule trivially (both halves present-tense, one identity with two qualities). Replaces the prior locked line, which moved to the About page header (see row #2). 2026-05-20: the prior phrasing "the agents handle the math" was swapped to "the agents handle the loops" — `math` risked reading as math-shame (Sean understands math; the line wasn't about that), and `loops` carries Karpathy-coded technical authority while naming what agents do that humans physically cannot sustain (10,000-step inference iteration). The 2-word cadence match preserves the tagline's rhythm. |
| **About page header (lead)** | *"Raised by Saturday morning cartoons and Vercel deployment logs."* | RETIRED FROM THE PAGE 2026-08-08 (Sean's preview review round 2: the About v3 opening spread puts the paragraph beside the character with no title; the line survives in the About `description` and the meta-description lock below) — was LOCKED 2026-05-18 | Sean drafted the actual About body and discovered the line worked better here than on the hero. "Raised by..." establishes lineage explicitly on the page where lineage belongs — biography. Pairs with the new home hero, which is now free to declare the role without doing biographical work. The two pages tell one story across two surfaces — declarative on home, biographical on About — without rhyming verbatim. Earns the home hero's "taste" claim by showing where the taste came from. |
...
| **Meta description / OG card / Substack tagline** | *"AI Product Manager. Raised by Saturday morning cartoons and Vercel deployment logs."* | LOCKED 2026-05-18 | The "Raised by..." pairing is the more concrete, more memorable copy for social-share previews and search results (more searchable nouns than "loops + taste"). The "AI Product Manager." prefix is SEO scaffolding — pattern-match for hiring-manager queries. The About-header line gets reused here as meta WITHOUT becoming a verbatim duplicate (the role prefix differentiates it). (2026-08-08: the hero is now the statement hero — see the row above — so the meta description and the on-page hero no longer share copy at all; this lock is unchanged by the recut.) Lives in `site-chrome-spec-v1.md` §11 `SITE_DESCRIPTION` constant; imported by `<SiteFooter.astro>`, `src/pages/contact.astro`, and the BaseLayout `<head>` meta tags. |
...
**Decision: E (the duet) is locked.** *(2026-08-08 note: the duet survives the statement-hero recut — the dateline strip still sits above the name and earns the screen through evidence; only the tagline slot below changed to the plain statement per §4 row 1.)* Three reasons: (1) it earns the headline through *evidence* (real agent activity) instead of self-claim — which is exactly the credibility the killed "ten years" line was trying and failing to provide; (2) it's automatable — the dateline reads from a real file the Daily Driver agent writes each morning, making the page *live*; (3) it anchors the wire-service voice as connective tissue across the whole site.
...
- **The 5 projects (locked):** 2D Animation Pipeline (`ACTIVE`), Code Brain (`ACTIVE`), Intent Engineering MCP (`SHIPPED` — amended 2026-05-17, was `COMING`; npm + MCP registry live since 5/12), The Block — Campus + RevOps (`ARCHIVED`), 16BitFit Battle Mode (`PAUSED`).
...
### 6.3 Interactive Explainer Pattern v1 — `docs/specs/interactive-explainer-pattern-v1.md`

**Status:** Shipped (animation-pipeline pilot; live render to confirm on `npm run dev`).

The non-obvious calls:
- **The explainer becomes a thing you *do*, not just read.** The static case-study diagram is replaced (opt-in via `explainer.interactive`) by a real graphite **run cycle** the reader **inks in by scrubbing a 10-phase timeline** — the human keyframe is pre-drawn, the fleet inks the in-betweens left-to-right, the shipped frame lands with a stamp-amber stamp. It makes the thesis tactile.
- **Author with AI, ship your own code.** The art is generated by **Gemini Nano Banana 2** as one consistency-locked run-cycle sheet, keyed to transparent, and self-hosted — *not* a live embedded Claude artifact (which would carry Anthropic chrome, an external CSP, and a runtime dependency). This is now a named DESIGN.md rule.
- **Graphite stays authentic; color-as-actor lives in the labels** (HUMAN `--ink` / FLEET `--teal` / SHIP `--success-teal`) + the stamp-amber stamp.
- **Progressive-enhancement, lightweight, no deps.** No-JS / reduced-motion shows the complete run with the scrubber hidden; CLS-safe; native `<input type="range">` for free keyboard support.
- The spec doubles as a **build journal** (brainstorm → skills → inspiration → Nano Banana 2 → keying → wiring) and carries a **§11 playbook** for rolling the pattern out to the other four explainers (generalize the component first).
