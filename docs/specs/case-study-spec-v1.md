...
>   - **`explainer:` frontmatter shape** (added to the `work` schema in `src/content/config.ts`, **optional** — the band renders only when present, so a project with no `explainer:` block simply skips it):
>     ```yaml
>     explainer:
>       src: /assets/projects/explainers/<slug>.webp   # required
...
>   - **Interactive explainers (2026-06-21):** all five case-study explainers are now progressively-enhanced interactive islands — an optional `explainer.interactive` key swaps the static `<img>` for a reader-driven component (a scrub, dial, toggle/switchboard, or a Game Boy drift-loop), per [`interactive-explainer-pattern-v1.md`](interactive-explainer-pattern-v1.md) + [`interactive-explainer-rollout-v1.md`](interactive-explainer-rollout-v1.md). The static `explainer.src` `.webp` stays the no-JS / reduced-motion fallback and the alt-text source; an unknown `interactive:` value degrades to the `<img>`.
