...
## NOTE-5 — Task 8 essays (lock #7) reconciliation

- Schema amendment (jdUrl optional + isNegativeSpace) committed separately first (bfd04a5).
- Frontmatter reconciled to lock #7: title "Meaning Over Access" → **"Access Over Meaning"** (the ironic title/slug split per §2.2.8 — drives the Task 14 OG fix), dateline/published → 2026-06-19, **status: DRAFT** (D5), +agent-infrastructure tag, roleMap Manus row → isNegativeSpace:true (dropped jdUrl), lastValidated 2026-06-19, crossPostedTo Substack.
- **On-disk-reality adjustments to lock #7 cross-links:** lock #7 assumed vault-knowledge-mcp + vault-synthesizer-eval-suite weren't shipped (null). Both ledger rows now EXIST (D11 union + Task 7), so: quadrantLegend.eval → vault-synthesizer-eval-suite; plottedArtifacts expanded to 4 (intent, vault-knowledge-mcp, phase-d, vault-synthesizer-eval-suite) — all resolve + reverse-render; relatedArchitecture kept [vault-scorecard] (exists on disk). Logged because this diverges from lock #7's literal null list, but is more correct given the union.
- **Resolved NOTE-2 flag:** HybridRouter removed from essays body prose ("via HybridRouter" → "via local-cloud routing"). Ceiling clean.
- **Deviation (Option-B-consistent):** essays renders the INLINE 5/24 manifesto body via `<Content />` (sourceUrl/explanationUrl null), NOT the upstream canonical fetch. Same rationale as architecture BLOCKER-1: the `[slug].astro:120` set:html path + the essay-bodies/ fetch-cache-inside-collection-glob would re-trigger the build break. The inline body is complete + voice-coherent; Sean to decide in v1.1 whether to wire the live canonical once the fetch/render infra is fixed. **The inline body is the 5/24 draft, not the 2026-05-21 voice-passed upstream — Sean voice-review recommended.**
- Footer-fold staged per §1.5: "subscribe via RSS" now hidden until a PUBLISHED essay exists (DRAFT → 2 affordances).

## NOTE-6 — Task 13 tile integration + A-4 art divergence (⛔ Sean voice/visual review)

- 5 re-authored tiles optimized 1122×1402 PNG → 800×1000 WebP (4:5, `scripts/phase-0/prep_project_tiles.py`, quality 82, ~1.05 MB total) overwriting the superseded 5/24 set. `hero_media` paths already `/assets/projects/<slug>.webp` (unchanged); only `hero_media_alt` rewritten to match the delivered art.
- **A-1 (animation-pipeline):** delivered art is a warrior character-design sketch (hooded figure + spear + amber ribbon, 3 margin pose-studies), NOT the prior "Claude mascot walk-cycle strip." Alt rewritten to the Anima art per Task 13 step 4.
- **A-2 / A-3 / A-5:** delivered art also differs from the prior alt strings (A-2 = night-desk laptop scene w/ 4 corner mascots; A-3 = child in a tent drawing a red pixel-creature to life — ties to "Drawing up agents to act with intent"; A-5 = hand-drawn POV Game Boy "16BitFit", no longer the photographic version). Alts rewritten to match.
- **⛔ A-4 (the-block) — HOLD POINT, surfaced to Sean.** Lock #9 rewrote A-4 alt to a "multi-surface PM composite" (3 Claude Skill cards + RevOps pipeline diagram + x402/A2A/MCP memo + bi-weekly P&E status bar). The **delivered A-4 tile depicts something different**: a figure seen from behind at a 4-monitor financial-charts trading desk in a wood-paneled study (bookshelves, armillary sphere, globe, brass lamp, window onto a domed civic building). I wrote accurate alt for the *delivered* art (alt must describe the actual image), but flag for Sean: (a) is this the intended A-4 tile, or should the composite be re-authored? (b) if the tile stays, the lock #9 alt string is now superseded by the delivered-art alt — confirm.
