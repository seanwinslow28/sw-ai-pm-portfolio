...
<InvestigationBoard>
  ...
  <BoardArtifact
    ...
    excerpt="Phase 6/6 shipped. Producer: SessionEnd flush → 02:30 synthesize. Consumer: SessionStart re-inject as additionalContext. Critic splits to Codex CLI + Anti-Gravity CLI in parallel so the variance is real. The vault now serves itself back into the next morning's context window.">
  ...
  <BoardArtifact
    ...
    excerpt="Typed-edge SQLite layer shipped. Six relation types: supports, contradicts, evolved_into, supersedes, depends_on, related_to. Weekly knowledge-lint contradiction detection collapses from an LLM scan to a single SQL query.">
  ...
  <BoardArtifact
    ...
    excerpt="Repo initialized Feb 6. The Obsidian vault predates it by years; the agent fleet starts at this commit. Everything else on this board grew from here.">
