...
- **2026-06-16 (critique W2 — minimal corner nav + projects dateline + annotation removal, §2.1 + §3.2):** Four affordance changes from the hiring-manager critique pass. **(a)** `HomeCornerNav.astro` added to the home page — a fixed top-right corner nav (wordmark + `/work/` + `/about/` links) that gives recruiters an immediate escape route without breaking the hero's no-sticky-SiteNav contract; `noChrome={true}` still suppresses the full SiteNav, the corner nav is an additive overlay. **(b)** The projects-section dateline label (`UPDATED · <date>`) was enlarged so the freshness signal is readable at arm's length. **(c)** The "updated weekly" pencil annotation was removed from the projects section — it was redundant noise next to the live dateline and undersold the daily-dated fleet refresh; `rev-3` marker and registration mark remain. **(d)** The teaser swipe affordance (`← SWIPE ME`) was repositioned to the right of the deck (was inline-below), made visible on touch devices (was desktop-only), and gained a card-stack peek cue so the deck reads as swipeable on first impression. See [CHANGELOG.md](../../CHANGELOG.md).
